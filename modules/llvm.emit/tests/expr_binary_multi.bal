...
function exprBinaryMulti() returns Module {

    Builder builder = new ();
    Module m = new ("x86_64-pc-linux-gnu");
    Function multi = m.appendFunction("@llvm.smul.with.overflow.i64", "{i64,i1}", functionAttributes = 
    ...
    Value R7 = builder.call(multi,[R5, R6]);
...
@test:Config {}
function testExprBinaryMulti() returns error? {
    Module m = exprBinaryMulti();
    string outputPath = check file:joinPath(file:getCurrentDir(), "modules", "llvm.emit", "tests", "expr_binary_multi.ll");
