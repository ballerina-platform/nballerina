...
function codeGenWhileStmt(StmtContext cx, bir:BasicBlock startBlock, BindingChain? initialBindings, s:WhileStmt stmt) returns CodeGenError|StmtEffect {
    ...
    if trueMerger is () && falseMerger is TypeMerger {
        ...
        return finishCodeGenWhileStmt(cx, initialBindings, stmt, loopHead, trueMerger, startRegister, regionIndex);
    }
        ...
        return finishCodeGenWhileStmt(cx, initialBindings, stmt, loopHead, trueMerger, startRegister, regionIndex, exit);
    }
    panic err:impossible();
}

// JBUG #35748 had to be broken into a separate function
function finishCodeGenWhileStmt(StmtContext cx, BindingChain? initialBindings, s:WhileStmt stmt, bir:BasicBlock loopHead, TypeMerger loopBodyMerger, int startRegister, bir:RegionIndex? regionIndex, bir:BasicBlock? exit = ()) returns CodeGenError|StmtEffect {
    ExprContext ec = cx.exprContext(initialBindings);
...
function codeGenMatchStmt(StmtContext cx, bir:BasicBlock startBlock, BindingChain? initialBindings, s:MatchStmt stmt) returns CodeGenError|StmtEffect {
    ...
    foreach int clauseIndex in 0 ..< stmt.clauses.length() {
        ...
        var { block: stmtBlockEnd, bindings: blockBindings } = check codeGenScope(cx, stmtBlock, clauseBindings[index] ?: initialBindings, clause.block);
        if stmtBlockEnd == () {
            continue;
        }
        else {
