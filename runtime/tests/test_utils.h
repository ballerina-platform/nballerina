...
static int min(int n1, int n2) { return (n1 > n2) ? n2 : n1; }

static int sign(int64_t n) { return n == 0 ? 0 : (n < 0 ? -1 : 1); }

double randDouble(bool isNan) {
    // assumeing two rands are enough to make mantissa and sign, ie:
    // popcount(RAND_MAX) * 2 >= 52 + 1
    uint64_t largeRand =
        ((uint64_t)rand() << __builtin_popcount(RAND_MAX)) + rand();
