...
void genAndValidateDecNeg(const char *dec, const char *decMinus) {
    TaggedPtrPanicCode tp = _bal_decimal_neg(_bal_decimal_const(dec));
    validateDec(tp.ptr, decMinus);
}

void testNeg() {
    genAndValidateDecNeg("0", "0");
    genAndValidateDecNeg("0.0", "0");
...
void testRem() {
    ...
    genAndValidateDecRem("9.999999999999999999999999999999998E6144", "1E-2", "", PANIC_ARITHMETIC_OVERFLOW);
    genAndValidateDecRem("9.05", "0.1", "0.05", 0);
    genAndValidateDecRem("9.0E31", "1E-2", "0", 0);
    genAndValidateDecRem("9.0E32", "1E-2", "0", PANIC_ARITHMETIC_OVERFLOW);
